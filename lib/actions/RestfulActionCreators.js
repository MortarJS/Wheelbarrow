...
export default class RestfulApiActions {
	 ...
	 * @param {function} RestfulHandler
	 *
	 * @return {null}
	 */
	// This shouldn't be `RestfulHandler` but ApiHandler extending this.handler
	// resourceAction: new RestfulAction('users', 'api.com/api/', MyDispatcher);
	// resourceAction.getResourceById(1, {});
	//
	...
	getResourceById(id, options) {
		...
		this.handler.get(url, options, this.dispatchResource);
	}
	...
	getCollection(options) {
		...
		this.handler.get(url, options, this.dispatchCollection);
	}
	...
	postResourceById(id, data, options) {
		let url = `${this.baseUrl}${this.collection}/${id}`; // config.apiUrl + version + this.collection
		this.handler.post(url, options, this.dispatchCollection);
	}
	...
	putResourceById(id, data, options) {
		let url = `${this.baseUrl}${this.collection}/${id}`; // config.apiUrl + version + this.collection
		this.handler.put(url, options, this.dispatchCollection)
	}
	...
	deleteCollection(options) {
		let url = `${this.baseUrl}${this.collection}/${id}`; // config.apiUrl + version + this.collection
		this.handler.delete(url, options, this.dispatchCollection)
	}
