...
export default class RestfulApiAction {
	 ...
	 */
	_constructEventName() {
		let name = `${this.collection}_${this.requestVerb}`;
		if (this.requestOptions.actionWithInclude &&
			this.requestOptions.modifiers &&
			this.requestOptions.modifiers.include) {

			name += `_WITH_${this.requestOptions.modifiers.include.collection}`;
		}
	...
	postToCollection(data, options) {
		...
		this.handler.post(url, data, options, this.dispatchResponse.bind(this));
	}
	...
	putCollection(data, options) {
		...
		this.handler.put(url, data, options, this.dispatchResponse.bind(this));
	}
